...
.avatar{width:40px;height:40px;border-radius:20px;background:#e8000d22;border:2px solid #e8000d44;display:flex;align-items:center;justify-content:center;color:#e8000d;font-weight:700;font-size:16px;flex-shrink:0;}
...
.sub{color:#444;font-size:11px;letter-spacing:5px;margin-bottom:36px;}
...
.online-dot{width:8px;height:8px;border-radius:4px;background:#00cc44;display:inline-block;margin-right:4px;}
</style>
</head>
<body>
...
<div class="screen" id="join">
  <div style="padding:40px 28px;flex:1;display:flex;flex-direction:column;justify-content:center;">
    <div class="logo">PARTEGEUR</div>
    <div class="sub">by REDrock</div>
    <div style="color:#777;font-size:13px;margin-bottom:24px;">No account needed. Just tell us who you are.</div>
    <input id="j-name" placeholder="Your name"/>
    ...
    <div style="color:#333;font-size:11px;text-align:center;margin-top:16px;">Messages vanish after being viewed once.<br/>No servers storing your data.</div>
  </div>
</div>
  ...
  <div class="scroll-list">
    ...
    <div style="background:#0f0f0f;border:1px solid #1a1a1a;border-radius:14px;padding:16px;margin-top:12px;">
      ...
      <div style="color:#555;font-size:12px;line-height:1.8;">
        • Messages are seen <span style="color:#888">once</span>, then deleted forever<br/>
        • Photos vanish after the receiver opens them<br/>
        • No account. No history. No traces.<br/>
        • Share this link with anyone to start chatting
      </div>
    </div>
...
function doJoin() {
  ...
  fetch('/api/join',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,contact,contactType})})
...
function refreshHome() {
  ...
  document.getElementById('home-greeting').textContent = 'Hey, '+currentUser.name;
...
function loadPeople() {
  fetch('/api/users').then(r=>r.json()).then(d=>{
    ...
    list.innerHTML = others.map(u=>`
      <div class="card" onclick="openChat(${JSON.stringify(u).replace(/"/g,'&quot;')})">
        <div class="avatar">${u.name[0].toUpperCase()}</div>
        <div style="flex:1;"><div style="color:#fff;font-weight:600;">${u.name}</div><div style="color:#444;font-size:12px;">${u.contactType==='phone'?'📞':'🏠'} ${u.contact}</div></div>
        <div style="color:#333;">›</div>
      </div>`).join('');
  });
}

function openChat(u) { chatTarget=u; document.getElementById('chat-name').textContent=u.name; document.getElementById('chat-avatar').textContent=u.name[0].toUpperCase(); showScreen('chat'); }
...
function loadDMs() {
  ...
  fetch(`/api/messages/dm/${currentUser.id}/${chatTarget.id}`).then(r=>r.json()).then(d=>{
    ...
    box.innerHTML = d.messages.map(m=>{
      ...
      return `<div class="bubble-wrap ${mine?'mine':''}"><div class="bubble ${mine?'mine':'theirs'}">${content}<div class="time">${timeAgo(m.ts)}${mine?' · '+(m.viewed?'seen 👁':'sent'):''}</div></div></div>`;
    }).join('');
...
function sendDM() {
  ...
  fetch('/api/messages/send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({fromId:currentUser.id,fromName:currentUser.name,toId:chatTarget.id,text,type:chatPhoto?'photo':'text',photo:chatPhoto})})
...
function loadGroups() {
  fetch('/api/groups').then(r=>r.json()).then(d=>{
    ...
    let html = '';
    if(mine.length) html += '<div class="section-label">MY GROUPS</div>'+mine.map(g=>`<div class="card" onclick="openGroup(${JSON.stringify(g).replace(/"/g,'&quot;')})"><div style="font-size:24px;">👥</div><div style="flex:1;"><div style="color:#fff;font-weight:600;">${g.name}</div><div style="color:#444;font-size:12px;">${g.members.length} members</div></div><div style="color:#333;">›</div></div>`).join('');
    if(others.length) html += '<div class="section-label" style="margin-top:20px;">JOIN A GROUP</div>'+others.map(g=>`<div class="card" onclick="joinGroup(${JSON.stringify(g).replace(/"/g,'&quot;')})"><div style="font-size:24px;">👥</div><div style="flex:1;"><div style="color:#888;font-weight:600;">${g.name}</div><div style="color:#333;font-size:12px;">${g.members.length} members · tap to join</div></div></div>`).join('');
...
function loadGrpMsgs() {
  ...
  fetch('/api/messages/group/'+currentGroup.id).then(r=>r.json()).then(d=>{
    ...
    box.innerHTML = d.messages.map(m=>{
      const mine = m.fromId===currentUser.id;
      let content = m.type==='photo'&&m.photo?`<img class="chat-img" src="${m.photo}"/>`:`<div>${m.text}</div>`;
      return `<div class="bubble-wrap ${mine?'mine':''}"><div>${!mine?`<div class="sender-name">${m.fromName}</div>`:''}<div class="bubble ${mine?'mine':'theirs'}">${content}<div class="time">${timeAgo(m.ts)}</div></div></div></div>`;
    }).join('');
    box.scrollTop = box.scrollHeight;
  });
}
...
function sendGroupMsg() {
  ...
  fetch('/api/messages/send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({fromId:currentUser.id,fromName:currentUser.name,groupId:currentGroup.id,text,type:grpPhoto?'photo':'text',photo:grpPhoto})})
...
function loadDMList() {
  fetch('/api/messages/conversations/'+currentUser.id).then(r=>r.json()).then(d=>{
    ...
    list.innerHTML = d.conversations.map(c=>{
      ...
      return `<div class="card" style="border-color:${unread?'#e8000d44':'#1e1e1e'}" onclick="openChat(${JSON.stringify(c.user).replace(/"/g,'&quot;')})">
...
socket.on('new_message', ()=>{ if(document.getElementById('chat').classList.contains('active'))loadDMs(); if(document.getElementById('group-chat').classList.contains('active'))loadGrpMsgs(); refreshHome(); });

// Splash
setTimeout(()=>{ document.getElementById('splash').classList.remove('active'); document.getElementById('join').classList.add('active'); }, 2000);
</script>
</body>
</html>
'''
...
def join():
    ...
    user = {'id': str(uuid.uuid4()), 'name': name, 'contact': data.get('contact', ''), 'contactType': data.get('contactType', 'phone'), 'joinedAt': int(time.time() * 1000)}
    users[user['id']] = user
...
def send_message():
    data = request.json
    msg = {'id': str(uuid.uuid4()), 'fromId': data.get('fromId'), 'fromName': data.get('fromName'), 'toId': data.get('toId'), 'groupId': data.get('groupId'), 'text': data.get('text', ''), 'type': data.get('type', 'text'), 'photo': data.get('photo'), 'viewed': False, 'ts': int(time.time() * 1000)}
    messages.append(msg)
...
def get_dm(user1, user2):
    conv = [m for m in messages if not m.get('groupId') and ((m.get('fromId') == user1 and m.get('toId') == user2) or (m.get('fromId') == user2 and m.get('toId') == user1))]
    for m in conv:
...
def get_conversations(user_id):
    ...
    result = []
    for uid, msgs in convs.items():
        if uid in users:
            result.append({'user': users[uid], 'messages': msgs})
...
def create_group():
    data = request.json
    group = {'id': str(uuid.uuid4()), 'name': data.get('name'), 'createdBy': data.get('createdBy'), 'members': [data.get('createdBy')], 'ts': int(time.time() * 1000)}
